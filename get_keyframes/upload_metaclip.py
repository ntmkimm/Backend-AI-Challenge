    ...
    for _id, path in tqdm(my_paths, desc=f"[GPU {rank}]"):
        try:
            ...
            if len(buffer['images']) >= BATCH_SIZE:
                if (device == "cpu"): print(device)
                with torch.no_grad():
                    inputs = torch.stack(buffer['images']).to(device)
                    embs = model.encode_image(inputs).cpu().tolist()
...
def main():
    ...
    schema = CollectionSchema([
        ...
        FieldSchema(name="metaclip_embedding", dtype=DataType.FLOAT_VECTOR, dim=DIMENSION),
    ...
    collection.create_index("metaclip_embedding", index_params)
    collection.load()
    ...
    end_time = time.time()
    print("Thoi gian embed xong: ", (end_time - start_time) / 60, " phut")
